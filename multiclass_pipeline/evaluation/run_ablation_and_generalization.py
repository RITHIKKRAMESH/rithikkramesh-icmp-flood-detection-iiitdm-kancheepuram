...
def normalize_labels(values):
    ...
    return labels.replace(
        {
            "NORMAL_ICMP": "benign", "NORMAL_TCP": "benign", "NORMAL_UDP": "benign",
            "normal_icmp": "benign", "normal_tcp": "benign", "normal_udp": "benign",
            "normal": "benign", "BENIGN": "benign", "benign": "benign", "Benign": "benign"
        }
    )

y_ext = normalize_labels(y_ext_raw)
encoder = joblib.load("models/label_encoder.pkl")
y_ext_encoded = encoder.transform(y_ext)
...
ab_models = {
    ...
    "XGBoost (Ablated)": XGBClassifier(n_estimators=100, max_depth=6, random_state=42, eval_metric="mlogloss"),
...
ab_results.append("ABLATION EXPERIMENT RESULT LOG: L3/L4 PACKET-HEADER SUBSET ONLY")
ab_results.append("=" * 80)
ab_results.append(f"Ablated Feature Subset (8 features): {ablated_features}\n")
...
for name, clf in ab_models.items():
    ...
    ab_results.append("-" * 80)
    ab_results.append(f"A2 Validation Accuracy: {val_acc*100:.2f}% | Macro F1-Score: {val_f1:.4f}")
    ab_results.append(f"B2 External Accuracy  : {ext_acc*100:.2f}% | Macro F1-Score: {ext_f1:.4f}")
    ab_results.append("\nExternal Confusion Matrix:")
    ab_results.append(str(confusion_matrix(y_ext_encoded, ext_preds)))
    ab_results.append("\nExternal Classification Report:")
    ab_results.append(classification_report(y_ext_encoded, ext_preds, target_names=encoder.classes_, digits=4, zero_division=0))
    ab_results.append("\n")
...
gen_results.append("GENERALIZATION STUDY RESULT LOG (DATASET B2)")
gen_results.append("=" * 80)
gen_results.append("SCENARIO A: Models trained on A2 (packet-level) evaluated on B2 (packet-level)\n")
...
for name, path in full_models.items():
    ...
    gen_results.append(classification_report(y_ext_encoded, preds, target_names=encoder.classes_, digits=4, zero_division=0))
    gen_results.append("\n")

# Scenario B: B2-trained models (evaluated on 20% test slice of B2)
gen_results.append("=" * 80)
gen_results.append("SCENARIO B: Models trained directly on B2 evaluated on B2 test split\n")
...
for name, path in full_models.items():
    ...
    gen_results.append("\nClassification Report:")
    gen_results.append(classification_report(y_test_b, preds, labels=range(len(encoder.classes_)), target_names=encoder.classes_, digits=4, zero_division=0))
    gen_results.append("\n")
